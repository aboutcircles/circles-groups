...
import "circles-contracts-v2/errors/Errors.sol";
...
import "circles-contracts-v2/proxy/ProxyFactory.sol";
import "circles-contracts-v2/treasury/IStandardVault.sol";

/// @dev Exactly the same code as StandardTreasury (unfortunately, inheritance is not possible due to missing virtual specifiers),
///      only added group as input for vault setup function.
contract StandardTreasuryOverriden is
    ERC165,
    ProxyFactory,
    TypeDefinitions,
    IERC1155Receiver,
    ...
    ICirclesCompactErrors
{
     ...
     */
    bytes4 public constant STANDARD_VAULT_SETUP_CALLPREFIX = bytes4(keccak256("setup(address,address)"));
     ...
     * @return vault Address of the vault
     */
    function _deployVault(address _group) internal returns (IStandardVault) {
        bytes memory vaultSetupData = abi.encodeWithSelector(STANDARD_VAULT_SETUP_CALLPREFIX, hub, _group);
