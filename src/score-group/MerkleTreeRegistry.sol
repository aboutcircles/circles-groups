 ...
 *      grace period, allowing consumers to accept proofs against either the current root or the recently
 *      replaced root.
 */
contract MerkleTreeRegistry {
     ...
     * @dev `previousRoot` is retained so proofs against the prior tree can remain valid for
     *      `ROOT_GRACE_PERIOD_BLOCKS` blocks after an update.
